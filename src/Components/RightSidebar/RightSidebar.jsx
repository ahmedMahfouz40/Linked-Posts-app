...
const RightSidebar = () => {
  ...
  function incSuggestionFollowers() {
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <FontAwesomeIcon
              icon={faUsers}
              className="text-[#1877f2] dark:text-[#63b3ff]"
            />
            <Link to={"/suggestions"}>
              <h3 className="text-base font-extrabold hover:underline text-slate-900 dark:text-white">
                Suggested Friends
              </h3>
            </Link>
          </div>
        ...
        <button
          onClick={incSuggestionFollowers}
          disabled={isFetching || limit >= 50}
          className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
        >
          {isFetching
            ? "Loading..."
            : limit >= 50
